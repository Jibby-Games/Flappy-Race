...
func save_settings(new_settings: Dictionary) -> void:
	...
	for setting in DEFAULT_SETTINGS.keys():
		...
		var value = new_settings[setting] if new_settings.has(setting) else DEFAULT_SETTINGS[setting]
...
func load_settings() -> Dictionary:
	...
	var new_settings = {}
	for setting in DEFAULT_SETTINGS.keys():
		new_settings[setting] = settings_file.get_value("settings", setting, DEFAULT_SETTINGS[setting])
